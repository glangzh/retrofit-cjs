...
// interceptors
export const AddReqInterceptor = (config, error, eject = false) => (target, property, descriptor) => {
    if (!descriptor) {
        _addReqInterceptor(target.axios || target.prototype.axios, config, error, typeof error === 'boolean' ? error : eject);
    }
}

export const AddResInterceptor = (response, error, eject = false) => (target, property, descriptor) => {
    if (!descriptor) {
